...
void deliverOrders(Company* c, Bakery* b) {
	vector<Employee*> employees = c->getEmployees();
	vector<Employee*>::iterator it = employees.begin();
	Employee* emp;
	while (it != employees.end()) {
		if ((*it)->getJob() == "deliveryman") {
			emp = (*it);
			b->deliverOrders(emp);
			cout << emp->getName() << " just delivered an order.";
			break;
		}
		it++;
	}
	...
	do {
		...
		switch (menuOption) {
		    ...
		    case 2:
				...
				break;
			case 3:
				increaseEmployeeByJob(c);
				break;
			case 4:
				showEmployeesByJob(c);
				break;
            default:
                cout << "ERROR: Not a valid Operation! Please try again..." << endl;
		}
...
int showEmployeeOptions(Company *c) {
	...
	cout << "3 - Increase employee salary by job" << endl;
	cout << "4 - Choose employee by job" << endl;
	cout << "0 - Back" << endl << endl;
	cout << "Option: ";
	cin >> menuOption;
